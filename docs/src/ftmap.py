...
ftmap.setAttribute('ft:originID', "Vitis_Libraries_xf_database")
ftmap.setAttribute('ft:title', "Vitis Libraries: xf_database")
...
def check_if_string_in_file(file_name, title, parentnode, prefix=""):
    ...
    with open(href, 'r', encoding="UTF-8", errors='ignore') as read_obj:
                ...
                if "toctree" in m.group():
                    ...
                    while j<len(data):
                        ...
                        if ".rst" in sub_line:
                            ...
                            if j == len(data)-1 or (j < len(data)-1 and ":start-after:" not in data[j+1]):
                                sub_files.append(sub_line.strip().replace(".. include:: ", ""))
                            j+=1
                        else:
                            break
                    i=j;
        ...
        for sub_file in sub_files:
            if "http" in sub_file:
                continue
            if ".rst" not in sub_file:
                sub_file = sub_file + ".rst" 
            ...
            if len(line) == 1:
                if not line[0].startswith('../'):
                    check_if_string_in_file(line[0], title, node, prefix)
                else:
                    prefix2 = ""
...
check_if_string_in_file("index.rst", "Vitis Libraries: xf_database", tocChild)
